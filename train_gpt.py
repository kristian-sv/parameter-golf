...
def compute_layer_sensitivities(
    ...
    """Compute per-layer sensitivity scores: tr(H_l) * ||W_l||_F^2.
    Higher score = more sensitive to quantization noise."""
    sensitivities: dict[str, float] = {}
    for name, tensor in state_dict.items():
        cat = _classify_param(name)
        if cat not in int6_cats or tensor.ndim < 2 or tensor.numel() <= 65536:
            continue
        ...
        if H is None:
            continue
        tr_H = float(torch.trace(H.float()).item())
        w_fro_sq = float(tensor.float().pow(2).sum().item())
        sensitivities[name] = tr_H * w_fro_sq
...
def allocate_mixed_precision(
    sensitivities: dict[str, float],
    state_dict: dict[str, Tensor],
    target_bytes: int = int(15.9 * 1024 * 1024),
) -> dict[str, int]:
    """Greedily allocate int5/int6/int7 per layer, keeping compressed size manageable.

    Strategy: start all at int6. Sort layers by sensitivity. Upgrade the top-N most
    sensitive to int7, downgrade the bottom-N least sensitive to int5, keeping the
    total bit budget roughly neutral (each upgrade is paired with a downgrade).

    Returns a dict mapping param name -> clip_range.
    """
    if not sensitivities:
        return {}

    # Sort layers by sensitivity (ascending = least sensitive first)
    sorted_layers = sorted(sensitivities.items(), key=lambda x: x[1])

    # Start all at int6
    allocation: dict[str, int] = {name: 31 for name in sensitivities}

    # Compute parameter counts for budget tracking
    param_counts: dict[str, int] = {}
    for name in sensitivities:
        param_counts[name] = int(state_dict[name].numel())

    # Pair upgrades with downgrades to keep total bits roughly neutral.
    # Each int5 saves 1 bit/param, each int7 costs 1 bit/param.
    # We pair them by parameter count so total storage stays close to int6-everywhere.
    n_layers = len(sorted_layers)
    # Try up to 1/3 of layers at each extreme
    max_changes = max(1, n_layers // 3)

    # Candidate downgrades (least sensitive) and upgrades (most sensitive)
    downgrade_candidates = sorted_layers[:max_changes]
    upgrade_candidates = sorted_layers[-max_changes:]

    # Greedily pair: downgrade least sensitive, upgrade most sensitive,
    # as long as the bit savings from downgrade >= bit cost of upgrade
    saved_bits = 0
    downgrades_applied = []
    upgrades_applied = []

    # First pass: compute all possible savings from downgrades
    for name, _sens in downgrade_candidates:
        bits_saved = param_counts[name]  # 1 bit per param saved
        downgrades_applied.append((name, bits_saved))
        saved_bits += bits_saved

    # Second pass: apply upgrades using saved bit budget
    for name, _sens in reversed(upgrade_candidates):
        bits_needed = param_counts[name]  # 1 bit per param cost
        if bits_needed <= saved_bits:
            upgrades_applied.append((name, bits_needed))
            saved_bits -= bits_needed
        else:
            break

    # Apply allocations
    for name, _ in downgrades_applied:
        allocation[name] = 15  # int5
    for name, _ in upgrades_applied:
        allocation[name] = 63  # int7
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None,
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            # Determine quantization type label from clip range
            if cr <= 15:
                qtype = "int5"
            elif cr <= 31:
                qtype = "int6"
...
def main() -> None:
    ...
    # Mixed-precision bit allocation based on Hessian sensitivity
    # BYPASSED for exp-000: uniform int6 baseline. Set MIXED_PRECISION=1 to re-enable.
    sensitivities = compute_layer_sensitivities(unbanked_sd, hessians, {"mlp", "attn"})
    target_mb = float(os.environ.get("TARGET_MB", "15.9"))
    if os.environ.get("MIXED_PRECISION", "0") == "1":
        clip_ranges = allocate_mixed_precision(sensitivities, unbanked_sd, target_bytes=int(target_mb * 1024 * 1024))
    else:
        clip_ranges = None
        log0("mixed_precision: DISABLED (uniform int6). Set MIXED_PRECISION=1 to enable.")
    if clip_ranges:
        int5_layers = [n for n, cr in clip_ranges.items() if cr == 15]
        int6_layers = [n for n, cr in clip_ranges.items() if cr == 31]
        int7_layers = [n for n, cr in clip_ranges.items() if cr == 63]
        log0(f"mixed_precision: {len(int5_layers)} int5, {len(int6_layers)} int6, {len(int7_layers)} int7 layers")
        for n in int5_layers:
            log0(f"  int5: {n} (sensitivity={sensitivities[n]:.4e})")
        for n in int7_layers:
            log0(f"  int7: {n} (sensitivity={sensitivities[n]:.4e})")
    else:
        log0("mixed_precision: no Hessian data, falling back to uniform int6")
    ...
    for name, info in quant_meta.items():
        if not (isinstance(info, dict) and info.get("type") in ("int5", "int6", "int7")): continue
